package scheduler.scheduling.strategy;

import mapcov.mapcalc;
import constraints.StartExploring;
...
public class MCRStrategy extends SchedulingStrategy {
	...
	private Queue<PSchedule> toExplore;
	...
	private PSchedule curSchedule;

	@Override
	public PSchedule getCurrentSchedule() {
	...
	public void startingExploration() {
		this.toExplore = new ConcurrentLinkedQueue<PSchedule>();
	...
	public void startingScheduleExecution() {
		...
		curSchedule = this.toExplore.poll();

		List<String> prefix = null;

		if (curSchedule != null){

			prefix = curSchedule.getCurSchedule();
		}
...
//		executeMultiThread(currentTrace, prefix);
	...
	private void executeSingleThread(Vector<String> prefix) {
		...
		/**
		 * update covered pattern info
		 */
		currentTrace.finishedLoading(true);

//		for (AbstractNode abstractNode : currentTrace.getFullTrace()) {
//			System.out.println(abstractNode);
//		}
		List<Pattern> patterns = mapcalc.getAllCoveredPatterns(currentTrace);
		List<Pattern> unCoveredPatterns = mapcalc.getUnCoveredPatterns(patterns);
		mapcalc.allCoveredPatterns.addAll(unCoveredPatterns);
		//for pattern
