...
import mapcov.LogUtil;
import mapcov.mapcalc;
import constraints.trace.Trace;
import scheduler.runtime.RVRunTime;
import scheduler.scheduling.MCRProperties;
...
public class ExplorationStatsListener extends ExplorationListenerAdapter {
    ...
    private static String EXPLORATIONSTATSLOGFILE = "";
    private static String FAILURETRACELOGFILE = "";

    public static volatile long numConstraints = 0;
    public static volatile long numFilterConstraints = 0;
    public static volatile long numUnSatConstraints = 0;
    public static volatile long constraintsTime = 0;
    public static volatile long unsatCoreTime = 0;
    public static volatile long numCheckPattern = 0;
    public static String bugTriggerPattern = "";
...
//        System.out.println(EXPLORATION_TIME + getDurationString(System.currentTimeMillis() - startTime));
        ...
        EXPLORATIONSTATSLOGFILE += "\n";
        EXPLORATIONSTATSLOGFILE += EXPLORATION_STATS_HEADER + "\n";
        EXPLORATIONSTATSLOGFILE += NUMBER_OF_SCHEDULES + numSchedules + "\n";
        EXPLORATIONSTATSLOGFILE += EXPLORATION_TIME + getDurationString(System.currentTimeMillis() - startTime) + "\n";
...
//        }
        if (failureDetected) {
            EXPLORATIONSTATSLOGFILE += FAILURE_DETECTED_MESSAGE + "\n";

            Logger logger = LogUtil.getLogger(Trace.appname);
            logger.info(String.valueOf(numSchedules)
                    + " " + numConstraints
                    + " " + numUnSatConstraints
                    + " " + numFilterConstraints
                    + " " + constraintsTime
                    + " " + unsatCoreTime
                    + " " + numCheckPattern
                    + " " + String.valueOf(System.currentTimeMillis() - startTime)
                    + " " + mapcalc.allCoveredPatterns.size()
                    + " " + bugTriggerPattern);
        }
...
//        }
        if (timeoutOccurred) {
            EXPLORATIONSTATSLOGFILE += "A timeout has occurred after " + getDurationString(timeoutValue) + "\n";
        }
//        System.out.println(EXPLORATION_STATS_FOOTER);
        EXPLORATIONSTATSLOGFILE += EXPLORATION_STATS_FOOTER + "\n";
        System.err.println(EXPLORATIONSTATSLOGFILE);
        System.err.flush();
//        
...
//        completedExploration();
        ...
        System.err.flush();
        FAILURETRACELOGFILE += "\n!!! FAILURE DETECTED DURING EXPLORATION OF SCHEDULE #" + numSchedules + ": " + ((errorMsg == null) ? "" : errorMsg) + "\n";
        FAILURETRACELOGFILE += "The following trace triggered this error:" + "\n";

        for (int i = 0; i < RVRunTime.failure_trace.size() ; i++) {

            FAILURETRACELOGFILE += "       " + RVRunTime.failure_trace.get(i) + "\n";
        }
        System.err.println(FAILURETRACELOGFILE);
    }
