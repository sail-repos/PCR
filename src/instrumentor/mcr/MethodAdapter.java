package instrumentor.mcr;

import instrumentor.instrumentation.Instrumentor;
import instrumentor.instrumentation.RVConfig;
import instrumentor.instrumentation.RVGlobalStateForInstrumentation;
import org.objectweb.asm.Label;
import org.objectweb.asm.MethodVisitor;
import org.objectweb.asm.Opcodes;
import org.objectweb.asm.Type;
import org.objectweb.asm.commons.AdviceAdapter;
import instrumentor.instrumentation.TargetClassInfo;
...
public class MethodAdapter extends AdviceAdapter implements Opcodes {
    ...
    private boolean isInit,isClinit,isSynchronized,isStatic, possibleRunMethod;
...
//            this.methodName = mname;
     ...
     * for COVCON
     */
    private static final String MethodEnter = "MethodEnter";
    private static final String MethodExit = "MethodExit";
    private static final String Object_String_Void = "(Ljava/lang/Object;Ljava/lang/String;)V";
...
//	    String sig_loc = ( sig_var + "|" + line_cur ).replace("/", ".");  //the above is too verbose
		...
		else
		{
		    switch (opcode) {
                ...
                case ATHROW:
                     ...
                     * for CovCon
                     */
                    if (TargetClassInfo.isUseCovCon() && !isInit && !isClinit){
                        addMethodMonitor(false);
                    }
                    mv.visitInsn(opcode);
                    break;
                default:
	...
	protected void onMethodEnter() {
         ...
         */
        if (TargetClassInfo.isUseCovCon() && !isInit && !isClinit){
            addMethodMonitor(true);
        }
		onMethodEnterNew();
	}

    private void addMethodMonitor(boolean isEnter){

        String fullMethodSign;
        String dotClassName = this.className.replace("/",".");
        if(dotClassName.equals(TargetClassInfo.getTargetClass()) || dotClassName.equals(TargetClassInfo.getTargetSuperClass())
                || TargetClassInfo.getTargetInnerClasses().contains(dotClassName)){

            if (dotClassName.equals(TargetClassInfo.getTargetClass()) || dotClassName.equals(TargetClassInfo.getTargetSuperClass())){

                fullMethodSign = TargetClassInfo.getTargetClass() + "." + methodName + "(" + getParamDesc(desc) + ")";
            }else{
                fullMethodSign = dotClassName + "." + methodName + "(" + getParamDesc(desc) + ")";
            }
            ...
            mv.visitLdcInsn(fullMethodSign);

            if(isEnter){

                mv.visitMethodInsn(Opcodes.INVOKESTATIC,Instrumentor.INSTR_EVENTS_RECEIVER,MethodEnter,Object_String_Void);
            }else{
                mv.visitMethodInsn(Opcodes.INVOKESTATIC,Instrumentor.INSTR_EVENTS_RECEIVER,MethodExit,Object_String_Void);
            }
        }
    }

    public static String getParamDesc(String paramDesc){

        String re="";
        Pattern pattern = Pattern.compile("\\((.*?)\\)");
        Matcher matcher = pattern.matcher(paramDesc);
        while (matcher.find()){
            re = matcher.group(1);
        }
        return re;
